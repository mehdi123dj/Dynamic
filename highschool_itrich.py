...
def main(): 
    ...
    for t in tqdm(G_t):
        ...
        #W=edge_constr(G_t[t])
        f = open(os.path.join(res_dir,"Input_W_highschool","W_day_%d_t0_%d.txt"%(d,t)), "w")
        f.write("day %d \t t_0= %d \n"%(d,t))
        for e in W.edges():
            u=e[0]
            v=e[1]
        ...
        f = open(os.path.join(res_dir,"RC_highschool","segmentation_day_%d_t0_%d.txt"%(d,t)), "w")
        f.write('t %d \n' %(t))
